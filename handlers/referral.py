...
async def referral(message: Message, cfg: Config, db: Database, premium: PremiumEmoji):
    ...
    # актуализируем username (если у тебя есть метод — оставь как было)
    try:
    ...
    # 🔒 рефералка доступна только Active
    if status != "active":
        text = (
            ...
            "💵 Награда: <b>2.5 USDT</b> за реферала\n\n"
            "📌 Реферал засчитывается, только когда он пополнит <b>5 USDT</b>."
        )
        await premium.answer_html(message, text)
        return

    # Active: показываем ссылку и реф. баланс
    ...
    joined = db.get_referrals_joined_count(tg_id)

    text = (
        "👥 <b>Реферальная система</b>\n\n"
        f"👤 <b>Рефералов зачислено:</b> <b>{referrals}</b>\n"
        f"👥 <b>Присоединилось по ссылке:</b> <b>{joined}</b>\n\n"
        "💵 <b>Награда:</b> <b>2.5 USDT</b> за реферала\n"
        "📌 Реферал засчитывается, когда приглашённый пополнит <b>5 USDT</b>\n\n"
        "🔗 <b>Ваша реферальная ссылка:</b>\n"
        f"<code>{ref_link}</code>\n\n"
        "✅ Отправьте ссылку друзьям — начисление произойдет автоматически после пополнения."
    )

    # ✅ Кнопка "Поделиться" (откроет выбор чатов и вставит готовый текст + ссылку)
    share_text = (
        "🚀 Присоединяйся к DigiBot и зарабатывай USDT!\n\n"
        "💵 Награда: 2.5 USDT за приглашённого друга"
    )
